...
#define AI_NET_OBJ_INSTANCE g_d_orig
 
#undef AI_D_ORIG_MODEL_SIGNATURE
#define AI_D_ORIG_MODEL_SIGNATURE     "1184d6827e11573385514aabf453942e"
...
#define AI_TOOLS_DATE_TIME   "Sat Dec 12 18:23:23 2020"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_D_ORIG_N_BATCHES
#define AI_D_ORIG_N_BATCHES         (1)
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                     1, 1, 4176, 1,
                     NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_D_ORIG_IN_NUM, &input_0_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_D_ORIG_OUT_NUM, &conv1d_11_output),
  &conv1d_layer, 0, NULL)



AI_DECLARE_STATIC
ai_bool d_orig_configure_activations(
  ai_network* net_ctx, const ai_buffer* activation_buffer)
{
  AI_ASSERT(net_ctx &&  activation_buffer && activation_buffer->data)

  ai_ptr activations = AI_PTR(AI_PTR_ALIGN(activation_buffer->data, AI_D_ORIG_ACTIVATIONS_ALIGNMENT));
...
AI_DECLARE_STATIC
ai_bool d_orig_configure_weights(
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_D_ORIG_MODEL_NAME,
      .model_signature   = AI_D_ORIG_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_d_orig_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_d_orig_create(
...
AI_API_ENTRY
ai_handle ai_d_orig_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_d_orig_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= d_orig_configure_weights(net_ctx, &params->params);
  ok &= d_orig_configure_activations(net_ctx, &params->activations);
...
AI_API_ENTRY
ai_i32 ai_d_orig_forward(ai_handle network, const ai_buffer* input)
{
