...
#define AI_NET_OBJ_INSTANCE g_wavenet18
 
#undef AI_WAVENET18_MODEL_SIGNATURE
#define AI_WAVENET18_MODEL_SIGNATURE     "6ee997bffb6d9005bb5208fcbfc7e6ec"
...
#define AI_TOOLS_DATE_TIME   "Fri Nov 20 19:33:56 2020"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_WAVENET18_N_BATCHES
#define AI_WAVENET18_N_BATCHES         (1)
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                     1, 1, 65888, 1,
                     NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_WAVENET18_IN_NUM, &input_0_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_WAVENET18_OUT_NUM, &conv1d_17_output),
  &conv1d_layer, 0, NULL)



AI_DECLARE_STATIC
ai_bool wavenet18_configure_activations(
  ai_network* net_ctx, const ai_buffer* activation_buffer)
{
  AI_ASSERT(net_ctx &&  activation_buffer && activation_buffer->data)

  ai_ptr activations = AI_PTR(AI_PTR_ALIGN(activation_buffer->data, AI_WAVENET18_ACTIVATIONS_ALIGNMENT));
...
AI_DECLARE_STATIC
ai_bool wavenet18_configure_weights(
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_WAVENET18_MODEL_NAME,
      .model_signature   = AI_WAVENET18_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_wavenet18_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_wavenet18_create(
...
AI_API_ENTRY
ai_handle ai_wavenet18_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_wavenet18_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= wavenet18_configure_weights(net_ctx, &params->params);
  ok &= wavenet18_configure_activations(net_ctx, &params->activations);
...
AI_API_ENTRY
ai_i32 ai_wavenet18_forward(ai_handle network, const ai_buffer* input)
{
