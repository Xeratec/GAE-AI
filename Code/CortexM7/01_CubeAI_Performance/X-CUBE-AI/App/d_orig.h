...
#ifndef __AI_D_ORIG_H__
#define __AI_D_ORIG_H__
#pragma once

#include "ai_platform.h"
#include "ai_platform_interface.h"

#define AI_D_ORIG_MODEL_NAME          "d_orig"

#define AI_D_ORIG_ACTIVATIONS_ALIGNMENT   (4)

#define AI_D_ORIG_IN_NUM       (1)
#define AI_D_ORIG_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 253, 1, 1, 1, NULL), \
}
#define AI_D_ORIG_IN_SIZE { \
  (253 * 1 * 1), \
}
#define AI_D_ORIG_IN_1_SIZE  (253 * 1 * 1)
#define AI_D_ORIG_IN_1_SIZE_BYTES  ((253 * 1 * 1) * 4)





#define AI_D_ORIG_OUT_NUM      (1)
#define AI_D_ORIG_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 1, 1, NULL), \
}
#define AI_D_ORIG_OUT_SIZE { \
  (1 * 1 * 1), \
}
#define AI_D_ORIG_OUT_1_SIZE  (1 * 1 * 1)
#define AI_D_ORIG_OUT_1_SIZE_BYTES  ((1 * 1 * 1) * 4)

#define AI_D_ORIG_N_NODES (12)
 ...
 * @ref ai_d_orig_create. Both the activations memory buffer 
 ...
 * header generated by the code generation tool. See AI_D_ORIG_*
 * defines into file @ref d_orig.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_d_orig_forward(
  ai_handle network, const ai_buffer* input);

AI_API_DECLARE_END

#endif /*__AI_D_ORIG_H__*/
